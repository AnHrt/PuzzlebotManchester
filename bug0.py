...
class GoToGoal():  
    def __init__(self):  
        ...
        self.y_target=4.5 #y position of the goal 
        ...
        self.target_position_tolerance=0.09 #target position tolerance [m] 
        fw_distance = 0.4  #0.7  distance to activate the following walls behavior [m] 
        ...
        while not rospy.is_shutdown():  
            ...
            if self.lidar_received: 
                ...
                if (self.at_goal()): #and (self.current_state == 'GoToGoal' or self.current_state == 'Clockwise' or self.current_state == 'CounterClockwise')):  
                    print("Goal reached") 
                    ...
                    if ((self.closest_range <= fw_distance) and (abs(thetaFWC - thetaGTG)<=np.pi/2)): 
                        # Implement the following walls behavior 
                        print(" change to following walls")
                        d_tao = d_t
                        self.current_state = "Clockwise"

                    elif((self.closest_range <= fw_distance) and (abs(thetaFWC - thetaGTG)>np.pi/2)): 
                        # Implement the following walls behavior 
                        print(" change to following walls")
                        d_tao = d_t
                        ...
                        print("Moving to the Goal: target (x,y)", self.x_targets[self.target], self.y_targets[self.target]) 
                    ...
                    if self.clear_shot(thetaAO,thetaGTG) and self.progress_with_fat_guards(d_t, d_tao, progress): 
                        self.current_state = 'GoToGoal' 
                        print("Change to Go to goal") 

                    else: 
                        print("Following wall: target (x,y)", self.x_targets[self.target], self.y_targets[self.target]) 
    ...
    def progress_with_fat_guards(self, d_t, d_tao, progress):
        if(d_t < (d_tao-progress)):
            return True
        else:
            return False
